...
public partial class admin_oweInfo : cAdmin
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            btnAdd.OnClientClick = Window1.GetShowReference("oweEdit.aspx", "新增欠费信息");
            BindGrid(0);
        }
    }
    ...
    private void BindGrid(int pageIndex)
    {
        ...
        sb.Append("owe_id is not null");
        try
        {
            GCGrid.RecordCount = com.getDataCount("web_owe ", sb.ToString(), DB_JOB);
            dt = com.getDataList(pageIndex, GCGrid.PageSize, "web_owe", "*", "owe_id", sb.ToString(), "order by owe_ID desc", DB_JOB);
        }
        catch (Exception ex)
        {
    ...
    protected void GCGrid_RowCommand(object sender, FineUIPro.GridCommandEventArgs e)
    {
        if (e.CommandName == "Delete")
        {
            ...
            if (SqlHelper.ExecuteNonQuery(SqlHelper.GetConnSting(DB_JOB), CommandType.Text, "delete from web_owe where owe_id =@id", para) > 0)
            {
