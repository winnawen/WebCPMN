...
public partial class admin_ownerComplain : cAdmin
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            btnAdd.OnClientClick = Window1.GetShowReference("ownerCpEdit.aspx", "新增业主投诉信息");
            BindGrid(0);
        }
    }
    private void BindGrid(int pageIndex)
    {
        ...
        sb.Append("cp_id is not null");
        try
        {
            GCGrid.RecordCount = com.getDataCount("web_cp ", sb.ToString(), DB_JOB);
            dt = com.getDataList(pageIndex, GCGrid.PageSize, "web_cp", "*", "cp_id", sb.ToString(), "order by cp_id desc", DB_JOB);
        }
        catch (Exception ex)
        {
    ...
    protected void GCGrid_RowCommand(object sender, FineUIPro.GridCommandEventArgs e)
    {
        if (e.CommandName == "Delete")
        {
            ...
            if (SqlHelper.ExecuteNonQuery(SqlHelper.GetConnSting(DB_JOB), CommandType.Text, "delete from web_cp where cp_id =@id", para) > 0)
            {
