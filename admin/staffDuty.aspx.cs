...
public partial class admin_staffDuty : cAdmin
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            btnAdd.OnClientClick = Window1.GetShowReference("staffDtEdit.aspx", "新增值班信息");
            BindGrid(0);
        }
    }
    ...
    protected void GCGrid_RowCommand(object sender, FineUIPro.GridCommandEventArgs e)
    {
        if (e.CommandName == "Delete")
        {
            ...
            if (SqlHelper.ExecuteNonQuery(SqlHelper.GetConnSting(DB_JOB), CommandType.Text, "delete from web_duty where duty_id =@id", para) > 0)
            {
