...
public partial class admin_staffInfo : cAdmin
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            btnAdd.OnClientClick = Window1.GetShowReference("staffEdit.aspx", "新增员工信息");
            BindGrid(0);
        }
    }
    ...
    private void BindGrid(int pageIndex)
    {
        ...
        sb.Append("staff_id is not null");
        try
        {
            GCGrid.RecordCount = com.getDataCount("web_staff ", sb.ToString(), DB_JOB);
            dt = com.getDataList(pageIndex, GCGrid.PageSize, "web_staff", "*", "staff_id", sb.ToString(), "order by staff_id desc", DB_JOB);
        }
        catch (Exception ex)
        {
    ...
    protected void GCGrid_RowCommand(object sender, FineUIPro.GridCommandEventArgs e)
    {
        if (e.CommandName == "Delete")
        {
            ...
            if (SqlHelper.ExecuteNonQuery(SqlHelper.GetConnSting(DB_JOB), CommandType.Text, "delete from web_staff where staff_id =@id", para) > 0)
            {
